...
class PlayerView: UIView, UIGestureRecognizerDelegate {
    ...
    // MARK: 音乐播放
    fileprivate var musicView = UIView() // 图片背景
    fileprivate var iconStrUrl:String = String() // 图片URL
    fileprivate var lrcStrUrl:String = String() // 歌词URL
    fileprivate var rotationImageView = UIImageView() // 图片UI
    fileprivate lazy var ircScrollView: LrcScrollView = LrcScrollView()
    
    
    
    // MARK:- 初始化
    init(frame: CGRect, contentView: UIView ,iconStr:String,lrcStr:String) {
    ...
    private func makeSubViews() {
        ...
        // 歌词
        if !iconStrUrl.isEmpty {
            musicView = UIView(frame:(CGRect(x: 0, y: playerLayer.frame.minY, width: playerLayer.frame.width, height: playerLayer.frame.height )))
            self.addSubview(musicView)
            
            
            if (player.status == AVPlayerStatus.readyToPlay) {
                
                let url = URL(string:self.iconStrUrl )
                
                //从网络获取数据流
                let data = try! Data(contentsOf: url!)
                //通过数据流初始化图片
                let newImage = UIImage(data: data)
                let blurImageView = UIImageView(image:newImage);
                musicView.addSubview(blurImageView)
                //                blurImageView.frame = musicView.frame
                blurImageView.snp.makeConstraints({ (make) in
                    make.top.equalTo(musicView)
                    make.left.equalTo(musicView)
                    make.right.equalTo(musicView)
                    make.bottom.equalTo(musicView)
                    
                })
                printLog("blurImageView"+" --- \(blurImageView.frame)")
                
                //首先创建一个模糊效果
                let blurEffect = UIBlurEffect(style: .light)
                //接着创建一个承载模糊效果的视图
                let blurView = UIVisualEffectView(effect: blurEffect)
                musicView.addSubview(blurView)
                ...
                printLog("blurView"+" --- \(blurView.frame)")
                //添加模糊视图到页面view上（模糊视图下方都会有模糊效果）
                
                
                // MARK: - 旋转的图片
                rotationImageView.kf.setImage(with: url)
                musicView.addSubview(rotationImageView)
                rotationImageView.snp.makeConstraints({ (make) in
                    make.width.equalTo(playerLayer.frame.height - 5)
                    make.height.equalTo(playerLayer.frame.height - 5)
                    make.center.equalTo(musicView)
                })
                
                if isFullScreen == false {
                    rotationImageView.layer.cornerRadius = playerLayer.frame.height * 0.5
                    
                } else {
                    rotationImageView.layer.cornerRadius = 210 * 0.5
                }
                rotationImageView.layer.masksToBounds = true
                rotationImageView.layer.borderWidth = 7;
                rotationImageView.backgroundColor = UIColor(r: 27, g: 27, b: 27)
                
                
                rotationImageView.layer.removeAnimation(forKey: "rotation")
                ...
                rotationImageView.layer.add(animation, forKey: "rotation")
            }
            
            
            
        }
        if !lrcStrUrl.isEmpty {
            //歌词
            musicView.addSubview(ircScrollView)
            ircScrollView.snp.makeConstraints({ (make) in
                make.width.equalTo(playerLayer.frame.height)
                make.height.equalTo(playerLayer.frame.height)
                make.left.equalTo(musicView.snp.left)
                make.centerY.equalTo(musicView.snp.centerY)
                
            })
            ircScrollView.lrcString = lrcStrUrl
        }
...
extension PlayerView {
    ...
    func fullScreenWithDirection(_ direction: RotateDirection) {
        ...
        playerLayer.frame = CGRect(x: 0, y: 0, width: kScreenH, height: kScreenW)
    ...
    func refreshShowValues() {
        ...
        if isDragging == false { // 当没有正在拖动进度时，才刷新时间和进度条
            ...
            ircScrollView.currentTime = currentTime
            // 显示时间
    ...
    func resizeTimeLabel() {
        ...
        let contantSize = CGSize(width: kScreenW, height: kScreenH)
        //左侧时间宽度
...
extension PlayerView {
    ...
    func playVideo() {
        ...
        rotationImageView.layer.resumeAnimation()
    ...
    func pauseVideo() {
        ...
        rotationImageView.layer.pauseAnimation()
