...
//    init(playerFrame: CGRect, contentView: UIView) {
    init(playerFrame: CGRect, contentView: UIView ,iconStr:String , lrcStr: String) {
        lrcString = iconStr

        super.init()
        
        playerView = PlayerView(frame: playerFrame, contentView: contentView ,iconStr:iconStr,lrcStr:lrcStr)
        playerView.delegate = self
        
        //计时器，循环执行(在视频暂停以及进入后台时会自动停止，恢复后自动开始)
        if !lrcStr.isEmpty {
            playbackTimeObserver = playerView.player.addPeriodicTimeObserver(forInterval: CMTimeMakeWithSeconds(0.1, Int32(NSEC_PER_SEC)), queue: DispatchQueue.main) { [weak self] (time) in
